const { makeFetch } = require('./common.js');

const baseURL = 'http://localhost:3000';

beforeEach(() => {
    // Reset the local airport to BRS before each test
    return makeFetch(`${baseURL}/airports/local`, { method: 'PUT', body: { airport: { iataCode: 'BRS' } }});
});
...
describe('PUT /airports/local', () => {
    it('should update what my local airport is', async () => {
        const res = await makeFetch(`${baseURL}/airports/local`, { method: 'PUT', body: { airport: { iataCode: 'LGW' } }});
        expect(res.status).toBe(200);
